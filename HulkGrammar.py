...
Stat_list, Expression, Stat, def_func, def_type, def_protocol, arg_list, TypeA, FuncBlock, IdT, BlockExpression, IdA, Inher, TypeBlock, BaseType, CallExpression, atrib_list, asig, Ext, rule_list, BoolExpression, Predicate, ArithExpr, TypeTestExpr,  Termino, UnarySub, Factor, PrimitiveExpression, VectorLiteral, Acces, LetExpression, DestAsig, CondExpr, WhileExpr, ForExpr, DownExpr, TypeInst, expr_list, asig_list, OptionalCondition, Iterator, arg, File, TopLevel, Prototype, Members, ProtocolBlock, StructuredExpression, BasicExpression, S, Artih_1= G.NonTerminals('Stat_list Expression Stat def_func def_type def_protocol arg_list TypeA FuncBlock IdT BlockExpression IdA Inher TypeBlock BaseType CallExpression atrib_list asig Ext rule_list BoolExpression Predicate ArithExpr TypeTestExpr Termino UnarySub Factor PrimitiveExpression VectorLiteral Acces LetExpression DestAsig CondExpr WhileExpr ForExpr DownExpr TypeInst expr_list asig_list OptionalCondition Iterator arg File TopLevel Prototype Members ProtocolBlock StructuredExpression BasicExpression S Arith_1')
...
TypeA %= dos_p + Id, lambda s: s[2], None, None
TypeA %= G.Epsilon, lambda s: None
FuncBlock %= BlockExpression, lambda s: s[1], None
FuncBlock %= flecha + Expression + punto_c, lambda s: s[2], None, None, None

def_type %= type_ + IdA + Inher + TypeBlock, lambda s: NodeType(s[2], s[3], s[4], s[1]), None, None, None, None

IdA %= Id + opar + arg_list + cpar, lambda s: NodePro_func(s[1], s[3], None), None, None, None, None
IdA %= Id, lambda s: s[1], None

Inher %= inherits + Id, lambda s: [s[2]], None, None
Inher %= inherits + Id + opar + arg + cpar, lambda s: [s[2], *s[4]], None, None, None, None, None
Inher %= G.Epsilon, lambda s: []
...
atrib_list %= atrib_list + Id + opar + arg_list + cpar + TypeA + FuncBlock, lambda s: [*s[1], NodeFunction(s[2], s[4], s[6], s[7])], None, None, None, None, None, None, None
...
BasicExpression %= Expression + and_ + BoolExpression, lambda s: NodeAnd(s[1], s[3], s[2]), None, None, None
BasicExpression %= Expression + or_ + BoolExpression, lambda s: NodeOr(s[1], s[3], s[2]), None, None, None
BasicExpression %= BoolExpression, lambda s: s[1], None

BoolExpression %= not_ + BoolExpression, lambda s: NodeUnary_Not(s[2], s[1]), None, None
BoolExpression %= Predicate, lambda s: s[1], None

Predicate %= ArithExpr + menor + ArithExpr, lambda s: NodeLess(s[1], s[3], s[2]), None, None, None
Predicate %= ArithExpr + menor_e + ArithExpr, lambda s: NodeLess_E(s[1], s[3], s[2]), None, None, None
Predicate %= ArithExpr + mayor + ArithExpr, lambda s: NodeGreater(s[1], s[3], s[2]), None, None, None
Predicate %= ArithExpr + mayor_e + ArithExpr, lambda s: NodeGreater_E(s[1], s[3], s[2]), None, None, None
Predicate %= ArithExpr + equal + ArithExpr, lambda s: NodeEqual(s[1], s[3], s[2]), None, None, None
Predicate %= ArithExpr + distinto + ArithExpr, lambda s: NodeDistint(s[1], s[3], s[2]), None, None, None
Predicate %= TypeTestExpr, lambda s: s[1], None
Predicate %= ArithExpr, lambda s: s[1], None

ArithExpr %= ArithExpr + arroba + Artih_1, lambda s: NodeConcat_s(s[1], s[3], s[2]), None, None, None
ArithExpr %= ArithExpr + arroba_d + Artih_1, lambda s: NodeConcat_d(s[1], s[3], s[2]), None, None, None
ArithExpr %= Artih_1, lambda s: s[1], None



Artih_1 %= Artih_1 + plus + Termino, lambda s: NodePlus(s[1], s[3], s[2]), None, None, None
Artih_1 %= Artih_1 + minus + Termino, lambda s: NodeMinus(s[1], s[3], s[2]), None, None, None
Artih_1 %= Termino, lambda s: s[1], None

Termino %= Termino + star + UnarySub, lambda s: NodeStar(s[1], s[3], s[2]), None, None, None
Termino %= Termino + div + UnarySub, lambda s: NodeDiv(s[1], s[3], s[2]), None, None, None
Termino %= Termino + resto + UnarySub, lambda s: NodeResto(s[1], s[3], s[2]), None, None, None
Termino %= UnarySub, lambda s: s[1], None

UnarySub %= minus + Factor, lambda s: NodeUnary_Minus(s[2], s[1]), None, None
UnarySub %= Factor, lambda s: s[1], None

Factor %= PrimitiveExpression + pow_ + Factor, lambda s: NodePow(s[1], s[3], s[2]), None, None, None
#Factor %= PrimitiveExpression + star + Factor, lambda s: NodePow(s[1], s[4], s[2]), None, None, None, None
...
BlockExpression %= llave_a + expr_list + punto_c + llave_c, lambda s: NodeBlockExpression(s[2]), None, None, None, None
expr_list %= expr_list + punto_c + Expression, lambda s: [*s[1], s[3]], None, None, None
expr_list %= Expression, lambda s: [s[1]], None
...
asig_list %= asig_list + coma + IdT + asignar + Expression, lambda s: [*s[1], NodeAsign(s[3], s[5])], None, None, None, None, None
asig_list %= IdT + asignar + Expression, lambda s: [NodeAsign(s[1], s[3])], None, None, None

DestAsig %= Acces + asignar_d + Expression, lambda s: NodeDestAssigment(s[1], s[3]), None, None, None

CondExpr %= if_ + opar + Expression + cpar + Expression + OptionalCondition, lambda s: NodeCondition(s[3], s[5], s[6], s[1]), None, None, None, None, None, None

OptionalCondition %= elif_ + opar + Expression + cpar + Expression + OptionalCondition, lambda s: NodeCondition(s[3], s[5], s[6], s[1]), None, None, None, None, None, None
OptionalCondition %= else_ + Expression, lambda s: s[2], None, None

WhileExpr %= while_ + opar + Expression + cpar + Expression, lambda s: NodeWhile(s[3], s[5], s[1]), None, None, None, None, None
...
arg %= arg + coma + Expression, lambda s: [*s[1], s[3]], None, None, None
